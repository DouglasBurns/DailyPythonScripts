	...
	for variable in variables:
		...
		if args.from_previous_binning:
			for hist_info in histogram_information:
				...
				r = calculate_resolutions( variable, histogram_information = hist_info, bin_edges = bin_edges_vis[variable], channel=hist_info['channel'], res_to_plot = args.plotting )
		...
		elif 'tau' in variable:
			best_binning, histogram_information = get_best_binning( histogram_information , 0.1, 0.0, n_min, minimum_bin_width[variable], nice_bin_width[variable], plot_resolution=args.plotting )
		else:
		...
		for info in histogram_information:
			print("In {} channel".format(info['channel']))
			print("Events {}".format(info['N']))
			print("Purities {}".format(info['p_i']))
			print("Stabilities {}".format(info['s_i']))
			print("Resolutions {}".format(info['res']))
			...
			if args.plotting:
				...
				plotting_response( info, variable, info['channel'], bin_choices[variable] )
...
def get_best_binning( histogram_information, p_min, s_min, n_min, min_width, nice_width, x_min = None, is_NJet=False, plot_resolution=False ):
	...
	for hist_info in histogram_information:
		...
		resolutions         = calculate_resolutions(  var, histogram_information = hist_info, bin_edges=bin_edges, channel = hist_info['channel'], res_to_plot = plot_resolution )
...
def get_next_end( histogram_information, bin_start, bin_end, p_min, s_min, n_min, min_width, nice_width, is_NJet=False ): 
	...
	for hist_info in histogram_information:
		gen_vs_reco_histogram = hist_info['hist']
		ch = hist_info['channel']
		...
		for bin_i in range ( current_bin_end, len( reco_i ) + 1 ):
			...
			if p >= p_min and s >= s_min and n_reco >= n_min:
				...
				res = calculate_resolutions( var, histogram_information=hist_info, bin_edges=[x_low, x_high], channel = ch )
...
def calculate_resolutions(variable, histogram_information = None, bin_edges = [], channel = 'combined', res_to_plot=False ):
	'''
	Calculate the resolutions in the bins using the residual method
	'''
	f = File( histogram_information['file'] )
	fineBinEdges_hist2d = histogram_information['hist'].Clone()
