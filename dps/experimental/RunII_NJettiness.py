...
from dps.utils.file_utilities import file_exists
import math
...
def drawHistograms( dictionaryOfHistograms, uncertaintyBand, config, channel, variable, ptReweight=None ) :
	histograms_to_draw = [
	...
	toPtReweight = ''
	if ptReweight: toPtReweight = '_ptReweighted'

	# More histogram settings to look semi decent
	histogram_properties = Histogram_properties()
	histogram_properties.name                   = '{channel}_{variable}{ptReweight}'.format(channel = channel, variable=variable, ptReweight=toPtReweight)
...
if __name__ == '__main__':
	...
	for variable in [
		...
		for ptReweight in [True, False]:
			for channel in ['electron', 'muon']:

				if ptReweight:
					path_to_DF = 'data/normalisation/background_subtraction/13TeV/{variable}_ptReweight/VisiblePS/'.format( variable = variable )
				else:
				...
				normalisation_filePath = '{path}/central/{filename}'.format(path=path_to_DF,filename=normalisation_fileName) 
				if not file_exists(normalisation_filePath): continue

				normalisation_results  = read_tuple_from_file( normalisation_filePath )
				dict_histograms = getHistogramsFromNormalisationResults(normalisation_results, reco_bin_edges )
				...
				drawHistograms( dict_histograms, totalSystematicUncertainty, config, channel, variable, ptReweight )
